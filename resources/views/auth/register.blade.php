<x-auth-layout title="Register" section_title="Register" section_description="Register using your email">
    <form action="{{ route('auth.store') }}" method="POST" class="flex flex-col gap-4 mt-2">
        ...
        <button type="submit" class="bg-green-500 border text-white px-3 py-2 text-center gap-2 cursor-pointer mt-4">
            <span>Register</span>
        </button>

        <p class="text-zinc-600 text-sm text-center">
            Already have an account?
            <a href="{{ route('auth.login') }}" class="text-green-500 font-semibold underline">Login Here!</a>
        </p>
    </form>
</x-auth-layout>
